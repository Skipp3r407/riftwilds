...
| Category | Weapon / FLOATING_FOCUS |
| Rarity | EPIC |
| Affinity | SPIRIT |
| Purpose | Inventory + battle overlay |
| Canvas | 1024×1024 inventory · 256 icon · 512 overlay |
| Attachment | floatingFocus |
| File names | `spiritfire-lantern.png`, `spiritfire-lantern-icon.png`, `spiritfire-lantern-overlay-l.png`, `spiritfire-lantern-overlay-r.png` |

## Full art prompt

Create a completely original 2D fantasy pet weapon named Spiritfire Lantern.

Weapon class: FLOATING_FOCUS
Affinity: SPIRIT
Rarity: EPIC
Compatible anatomy: floatingFocus

The weapon has a distinctive silhouette matching Lantern focus burning with gentle spiritflame., constructed from original fantasy materials with controlled magical energy.

It attaches using floatingFocus.
...
Cropped iconic Spiritfire Lantern on transparent background, strong silhouette, readable at 48px.

## Overlay prompt

Battle overlay for floatingFocus, left and right facing, transparent, no clipping through pet face.

## Effect prompt

Attack VFX for Spiritfire Lantern: stylized non-graphic impact sparks matching SPIRIT affinity.
