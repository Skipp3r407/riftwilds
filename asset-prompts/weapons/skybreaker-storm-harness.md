# Skybreaker Storm Harness

| Field | Value |
|-------|-------|
| Name | Skybreaker Storm Harness |
| Category | Weapon / HARNESS |
| Rarity | LEGENDARY |
| Affinity | STORM |
| Purpose | Inventory + battle overlay |
| Canvas | 1024×1024 inventory · 256 icon · 512 overlay |
| Attachment | back |
| File names | `skybreaker-storm-harness.png`, `skybreaker-storm-harness-icon.png`, `skybreaker-storm-harness-overlay-l.png`, `skybreaker-storm-harness-overlay-r.png` |

## Full art prompt

Create a completely original 2D fantasy pet weapon named Skybreaker Storm Harness.

Weapon class: HARNESS
Affinity: STORM
Rarity: LEGENDARY
Compatible anatomy: back

The weapon has a distinctive silhouette matching Storm-forged harness that opens a sky rift ribbon., constructed from original fantasy materials with controlled magical energy.

It attaches using back.
...
Cropped iconic Skybreaker Storm Harness on transparent background, strong silhouette, readable at 48px.

## Overlay prompt

Battle overlay for back, left and right facing, transparent, no clipping through pet face.

## Effect prompt

Attack VFX for Skybreaker Storm Harness: stylized non-graphic impact sparks matching STORM affinity.
