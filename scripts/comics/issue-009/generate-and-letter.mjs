/**
 * Stage 1 + 2 for The Riftwright Issue #9:
 ...
 *
 *   node scripts/comics/issue-009/generate-and-letter.mjs
 *   node scripts/comics/issue-009/generate-and-letter.mjs --pages=1-5
...
const PUBLIC_PAGES = path.join(ROOT, "public/assets/comics/the-riftwright/issue-009/pages");
...
async function main() {
  ...
  for (const page of pages) {
    ...
    try {
      ...
      if (page.bookRole === "front-cover") {
        ...
        fs.copyFileSync(letteredPath, path.join(PUBLIC_COVERS, "09-the-riftwright.webp"));
        await sharp(letteredPath).png().toFile(path.join(PUBLIC_COVERS, "09-the-riftwright.png"));
  ...
  status.summary = {
    ...
    grokResume:
      "set XAI_API_KEY=... && node scripts/comics/issue-009/generate-and-letter.mjs --force",
  };
