/**
 * Stage 1 + 2 for The Storm King Issue #5:
 ...
 *
 *   node scripts/comics/issue-005/generate-and-letter.mjs
 *   node scripts/comics/issue-005/generate-and-letter.mjs --pages=1-5
 *   node scripts/comics/issue-005/generate-and-letter.mjs --letter-only
 *   node scripts/comics/issue-005/generate-and-letter.mjs --force
 */
...
const PUBLIC_PAGES = path.join(ROOT, "public/assets/comics/the-storm-king/issue-005/pages");
...
async function main() {
  ...
  for (const page of pages) {
    ...
    try {
      ...
      if (page.bookRole === "front-cover") {
        ...
        fs.copyFileSync(letteredPath, path.join(PUBLIC_COVERS, "05-the-storm-king.webp"));
        await sharp(letteredPath).png().toFile(path.join(PUBLIC_COVERS, "05-the-storm-king.png"));
  ...
  status.summary = {
    ...
    grokResume:
      "set XAI_API_KEY=... && node scripts/comics/issue-005/generate-and-letter.mjs --force",
  };
