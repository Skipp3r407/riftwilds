/**
 * Stage 1 + 2 for The Shattered Star Issue #10:
 ...
 *
 *   node scripts/comics/issue-010/generate-and-letter.mjs
 *   node scripts/comics/issue-010/generate-and-letter.mjs --pages=1-5
 *   node scripts/comics/issue-010/generate-and-letter.mjs --letter-only
 *   node scripts/comics/issue-010/generate-and-letter.mjs --force
 */
...
const PUBLIC_PAGES = path.join(ROOT, "public/assets/comics/the-shattered-star/issue-010/pages");
...
function atmosColors(atmosphere) {
  const map = {
    ...
    forge: { a: "#0e1218", b: "#2a2118", c: "#66e0ff", d: "#ff7a33" },
    celestial: { a: "#0a1830", b: "#1a3040", c: "#e8f4ff", d: "#ffd27a" },
    space: { a: "#050810", b: "#121a28", c: "#66e0ff", d: "#ffb84d" },
    void: { a: "#0a0a10", b: "#1a1018", c: "#66e0ff", d: "#c07070" },
    battle: { a: "#1a2030", b: "#3a2820", c: "#3de7ff", d: "#ffb84d" },
    memory: { a: "#1a2438", b: "#2a2118", c: "#b8d4ff", d: "#ffd27a" },
  };
  return map[atmosphere] || map.rift;
}
...
async function ensureFont() {
  ...
  // Comic Neue Bold â€” SIL OFL (not served from /public)
  const url =
  ...
  try {
    ...
    console.log("Downloaded Comic Neue Bold â†’ assets/fonts/comics/ (private)");
    return true;
  } catch (e) {
    console.warn("Font download failed â€” falling back to Georgia:", e.message);
    return false;
  }
}
...
async function main() {
  ...
  for (const page of pages) {
    ...
    try {
      if (!letterOnly) {
        if (!force && fs.existsSync(rawPath)) {
          ...
          if (process.env.XAI_API_KEY) {
            try {
              ...
              console.warn(`Grok failed p${page.pageNumber}: ${e.message} â€” procedural fallback`);
        ...
        // letter-only needs a base â€” synthesize
      ...
      if (page.bookRole === "front-cover") {
        ...
        fs.copyFileSync(letteredPath, path.join(PUBLIC_COVERS, "10-the-shattered-star.webp"));
        await sharp(letteredPath).png().toFile(path.join(PUBLIC_COVERS, "10-the-shattered-star.png"));
    ...
    if (page.pageNumber % 5 === 0) console.log(`â€¦ through page ${page.pageNumber}`);
  }
  ...
  console.log(`Status â†’ ${STATUS_FILE}`);
}
