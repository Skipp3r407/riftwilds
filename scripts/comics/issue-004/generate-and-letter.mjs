/**
 * Stage 1 + 2 for The Lost City Issue #4:
 ...
 *
 *   node scripts/comics/issue-004/generate-and-letter.mjs
 *   node scripts/comics/issue-004/generate-and-letter.mjs --pages=1-5
 *   node scripts/comics/issue-004/generate-and-letter.mjs --letter-only
...
const PUBLIC_PAGES = path.join(ROOT, "public/assets/comics/the-lost-city/issue-004/pages");
...
async function main() {
  ...
  for (const page of pages) {
    ...
    try {
      ...
      if (page.bookRole === "front-cover") {
        ...
        fs.copyFileSync(letteredPath, path.join(PUBLIC_COVERS, "04-the-lost-city.webp"));
        await sharp(letteredPath).png().toFile(path.join(PUBLIC_COVERS, "04-the-lost-city.png"));
  ...
  status.summary = {
    ...
    grokResume:
      "set XAI_API_KEY=... && node scripts/comics/issue-004/generate-and-letter.mjs --force",
  };
