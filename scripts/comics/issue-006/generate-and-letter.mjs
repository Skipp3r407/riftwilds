/**
 * Stage 1 + 2 for The Merchant's Secret Issue #6:
 ...
 *
 *   node scripts/comics/issue-006/generate-and-letter.mjs
 *   node scripts/comics/issue-006/generate-and-letter.mjs --pages=1-5
 *   node scripts/comics/issue-006/generate-and-letter.mjs --letter-only
 *   node scripts/comics/issue-006/generate-and-letter.mjs --force
 */
...
const ISSUE = path.join(ROOT, "content/comics/the-merchants-secret/issue-006");
const PUBLIC_PAGES = path.join(ROOT, "public/assets/comics/the-merchants-secret/issue-006/pages");
...
async function main() {
  ...
  for (const page of pages) {
    ...
    try {
      ...
      if (page.bookRole === "front-cover") {
        ...
        fs.copyFileSync(letteredPath, path.join(PUBLIC_COVERS, "06-merchants-secret.webp"));
        await sharp(letteredPath).png().toFile(path.join(PUBLIC_COVERS, "06-merchants-secret.png"));
  ...
  status.summary = {
    ...
    errors,
    resume:
      "node scripts/comics/issue-006/generate-and-letter.mjs --force --pages=1-10",
    grokResume:
      "set XAI_API_KEY=... && node scripts/comics/issue-006/generate-and-letter.mjs --force",
  };
