...
export function LiveWorldDialogueOverlay({
  ...
  const [credits, setCredits] = useState(() =>
    typeof window !== "undefined" ? loadLivePlayState().demoCredits : 200,
  );
  ...
  if (shopId) {
    ...
    return (
      <div className="absolute inset-x-0 bottom-0 z-30 p-3 md:p-5">
        <div className="w-full rounded-2xl border border-[var(--stroke)] bg-[rgba(10,14,24,0.94)] px-4 py-4 backdrop-blur-md md:px-6">
          <p className="font-display text-sm text-[var(--cyan)]">
            {shop?.title ?? "Shop"}
          </p>
          <p className="mt-1 text-xs text-[var(--text-muted)]">
            Demo credits: {credits} · No real SOL spent
          </p>
          {shopMsg ? (
              ...
              <li key={item.itemId} className="flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const result = purchaseFromShop(state, shopId, item.itemId);
  ...
  if (dialogue) {
    ...
    return (
      <div className="absolute inset-x-0 bottom-0 z-30 p-3 md:p-5">
        <div className="w-full rounded-2xl border border-[var(--stroke)] bg-[rgba(10,14,24,0.92)] px-4 py-4 text-left backdrop-blur-md md:px-6">
          <div className="flex gap-3">
            {dialogue.portraitAsset ? (
              ...
              <img
                ...
                width={64}
                height={64}
                className="h-16 w-16 shrink-0 rounded-lg border border-[var(--stroke)] object-cover bg-[rgba(20,24,40,0.8)]"
                onError={(e) => {
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (result.openShopId) {
                      ...
                      setCredits(loadLivePlayState().demoCredits);
                      return;
                    }
                    if (!result.dialogue) {
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-28 z-20 flex justify-center px-4 md:bottom-8">
      <div className="rounded-full border border-[var(--stroke)] bg-[rgba(8,12,22,0.78)] px-4 py-2 text-xs text-[var(--text-muted)] backdrop-blur-md">
        {prompt.label}
      </div>
    </div>
  );
