...
- **Riftling cries:** Original per-species WAVs from `scripts/assets/generate-riftling-cries.mjs` (procedural default; optional xAI/Grok TTS when `XAI_API_KEY` is set). **CC0 1.0** / original IP — not Pokémon cries or third-party packs. See `riftlings/MANIFEST.json`.
